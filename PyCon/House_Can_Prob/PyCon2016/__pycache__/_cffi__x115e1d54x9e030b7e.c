...
/**********/
        ...
        for (i=0; i<x-ncol+1; i++){
            for (j=0; j<y; j++){
                ...
                {
                    mat[i][j]=mat[i-1][j]-m[i-1][j]+m[i+ncol-1][j];
                }
            }
        }
        for (i=0; i<x; i++){
            for (j=0; j<y-nrow+1; j++){
                ...
                {
                    mat[i][j]=mat[i][j-1]-mat[i][j-1]+mat[i][j+nrow-1];
                }
...
static PyMethodDef _cffi_methods[] = {
  {"sum1", _cffi_f_sum1, METH_VARARGS, NULL},
...
PyMODINIT_FUNC
PyInit__cffi__x115e1d54x9e030b7e(void)
{
  PyObject *lib;
...
PyMODINIT_FUNC
init_cffi__x115e1d54x9e030b7e(void)
{
  PyObject *lib;
  lib = Py_InitModule("_cffi__x115e1d54x9e030b7e", _cffi_methods);
  if (lib == NULL)
    return;
  if (((void)lib,0) < 0 || _cffi_init() < 0)
    return;
