...
/**********/
        ...
        for (i=0; i<x; i++){
            for (j=0; j<y-nrow+1; j++){
                ...
                {
                    mat[i][j]=mat[i][j-1]-mat[i][j-1]+m[i][j+nrow-1];
                }
...
PyMODINIT_FUNC
PyInit__cffi__x41b55dbfx472865e2(void)
{
  PyObject *lib;
...
PyMODINIT_FUNC
init_cffi__x41b55dbfx472865e2(void)
{
  PyObject *lib;
  lib = Py_InitModule("_cffi__x41b55dbfx472865e2", _cffi_methods);
  if (lib == NULL)
    return;
  if (((void)lib,0) < 0 || _cffi_init() < 0)
    return;
