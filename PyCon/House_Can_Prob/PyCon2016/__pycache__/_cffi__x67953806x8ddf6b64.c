...
/**********/
        ...
        for (i=0; i<x-ncol+1; i++){
            for (j=0; j<y; j++){
                ...
                {
                    mat[i][j]=tmpsm-mat[i-1][j]+mat[i+ncol][j];
                }
...
PyMODINIT_FUNC
PyInit__cffi__x67953806x8ddf6b64(void)
{
  PyObject *lib;
...
PyMODINIT_FUNC
init_cffi__x67953806x8ddf6b64(void)
{
  PyObject *lib;
  lib = Py_InitModule("_cffi__x67953806x8ddf6b64", _cffi_methods);
  if (lib == NULL)
    return;
  if (((void)lib,0) < 0 || _cffi_init() < 0)
    return;
