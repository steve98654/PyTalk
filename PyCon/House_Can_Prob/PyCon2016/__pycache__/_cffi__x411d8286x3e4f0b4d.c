...
/**********/
        ...
        for (i=0; i<x-ncol+1; i++){
            for (j=0; j<y; j++){
                ...
                {
                    mat[i][j]=tmpsm-mat[i-1][j]+mat[i+nrow-1][j];
                }
...
PyMODINIT_FUNC
PyInit__cffi__x411d8286x3e4f0b4d(void)
{
  PyObject *lib;
...
PyMODINIT_FUNC
init_cffi__x411d8286x3e4f0b4d(void)
{
  PyObject *lib;
  lib = Py_InitModule("_cffi__x411d8286x3e4f0b4d", _cffi_methods);
  if (lib == NULL)
    return;
  if (((void)lib,0) < 0 || _cffi_init() < 0)
    return;
