...
/**********/
        ...
        for (i=0; i<x-ncol+1; i++){
            for (j=0; j<y; j++){
                ...
                {
                    mat[i][j] = tmpsum - mat[i-1][j] + mat[i+ncol-1][j];
                }
            }
        }
        for (i=0; i<x; i++){
...
PyMODINIT_FUNC
PyInit__cffi__x5051b783x56d77e1c(void)
{
  PyObject *lib;
...
PyMODINIT_FUNC
init_cffi__x5051b783x56d77e1c(void)
{
  PyObject *lib;
  lib = Py_InitModule("_cffi__x5051b783x56d77e1c", _cffi_methods);
  if (lib == NULL)
    return;
  if (((void)lib,0) < 0 || _cffi_init() < 0)
    return;
