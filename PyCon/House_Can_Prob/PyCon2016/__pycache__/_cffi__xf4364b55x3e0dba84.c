        ...
        for (i=0; i<x-ncol+1; i++){
            for (j=0; j<y; j++){
               ...
               // }
                ...
                //{
                //    tmpsm=0;
                //    mat[i][j]=tmpsm-0*m[i-1][j]+0*m[i+ncol-1][j];
               // }
...
PyMODINIT_FUNC
PyInit__cffi__xf4364b55x3e0dba84(void)
{
  PyObject *lib;
...
PyMODINIT_FUNC
init_cffi__xf4364b55x3e0dba84(void)
{
  PyObject *lib;
  lib = Py_InitModule("_cffi__xf4364b55x3e0dba84", _cffi_methods);
  if (lib == NULL)
    return;
  if (((void)lib,0) < 0 || _cffi_init() < 0)
    return;
