...
/**********/
        ...
        for (i=0; i<x-ncol+1; i++){
            for (j=0; j<y; j++){
                ...
                {
                    mat[i][j]=tmpsm-m[i-1][j]+m[i+ncol-1][j];
                }
...
PyMODINIT_FUNC
PyInit__cffi__x8407d21cx7bde2f98(void)
{
  PyObject *lib;
...
PyMODINIT_FUNC
init_cffi__x8407d21cx7bde2f98(void)
{
  PyObject *lib;
  lib = Py_InitModule("_cffi__x8407d21cx7bde2f98", _cffi_methods);
  if (lib == NULL)
    return;
  if (((void)lib,0) < 0 || _cffi_init() < 0)
    return;
