        ...
        for (i=0; i<x-nrow+1; i++){
            for (j=0; j<y; j++){
                ...
                else
                {
                    ...
                    printf("tmpsm %f",mat[i][j]);
                }
            }
        }
        for (i=0; i<x; i++){
            for (j=0; j<y-ncol+1; j++){
                tmpsm =0;
...
PyMODINIT_FUNC
PyInit__cffi__xa3b44f09xf1b8b53a(void)
{
  PyObject *lib;
...
PyMODINIT_FUNC
init_cffi__xa3b44f09xf1b8b53a(void)
{
  PyObject *lib;
  lib = Py_InitModule("_cffi__xa3b44f09xf1b8b53a", _cffi_methods);
  if (lib == NULL)
    return;
  if (((void)lib,0) < 0 || _cffi_init() < 0)
    return;
